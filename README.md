...
`equipment_kb` 模块已从「球拍库」升级为**三类装备（球拍/球鞋/羽毛球）的统一推荐系统**，
并预留 AI 智能体接口。目标是：让用户更好了解产品、选到适配自己的装备。

- 结构化数据：`equipment_kb/equipment.json`（品牌、价格、参数、打法、评分、评价、来源标注）
- 规则引擎：`equipment_kb/recommender.py`（水平+预算+打法三维匹配）
- AI 顾问：`equipment_kb/ai_advisor.py`（LLM 自然语言推荐，无 Key 自动回退）
- 产品架构与分阶段路线：见 [docs/PRODUCT_ROADMAP.md](./docs/PRODUCT_ROADMAP.md)

> 数据来自品牌官网 + 公开评测**整理**（标注来源），非实时爬取；当前为覆盖头部品牌的「样本库」。
...
实时摄像头链路是：固定摄像头 → TrackNet / YOLO 等检测器识别球心 → 通过场地标定得到像素/米 → 向 `/ws/sessions/{session_id}` 推送 `{"type":"point","x":123,"y":456,"timestamp":1720000000.12}`。大屏上的按钮是用于验收产品交互的模拟数据。

### 上传视频检测球速

网页的“上传视频检测球速”区域调用 `POST /api/video/analyze`。服务会读取视频 FPS，依据画面中的场地线估计单双打场地尺寸（13.40 × 5.18/6.10 米），追踪高亮移动目标，返回峰值/平均球速、检测率和“疑似杀球/快速平抽”等判断。

视频版本是轻量 CV MVP，适合固定机位、光线稳定、60fps 以上的短视频；普通手机视频可能因为羽毛球太小、运动模糊或遮挡而降低置信度，报告会同时给出标定来源和告警。复杂球馆环境建议把候选点算法替换为 TrackNet/YOLO，并用场地四角做透视标定。

视频报告还会给出峰值球速对应的近似扣杀角度；网页端会将最近 20 次分析保存在浏览器本地，并展示最近记录、最佳峰值和击球类型。点击“生成成绩分享卡”可以下载一张 PNG 数据卡，方便发给教练或球友。角度是基于视频画面轨迹的估计值，不等同于经过透视校正的专业测量。
